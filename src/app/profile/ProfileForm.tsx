...
export default function ProfileForm({ initialProfile }: { initialProfile: InitialProfile }) {
  ...
  return (
    <form onSubmit={saveProfile} className="mt-8 space-y-6">
      ...
      <fieldset className="space-y-4 border border-foreground/15 p-4"><legend className="px-2 font-mono text-xs uppercase">Location</legend><label className="block">How should location be used?<select className={inputClass} value={form.locationSource} onChange={(event) => update("locationSource", event.target.value)}><option value="unset">Do not provide it yet</option><option value="approximate">Approximate city / neighbourhood</option><option value="precise">Precise GPS coordinates</option></select></label><label className="block">City or neighbourhood<input className={inputClass} value={form.city} onChange={(event) => update("city", event.target.value)} /></label>{form.locationSource === "precise" && <div className="grid gap-4 sm:grid-cols-2"><label>Latitude<input className={inputClass} inputMode="decimal" value={form.latitude} onChange={(event) => update("latitude", event.target.value)} /></label><label>Longitude<input className={inputClass} inputMode="decimal" value={form.longitude} onChange={(event) => update("longitude", event.target.value)} /></label></div>}</fieldset>
